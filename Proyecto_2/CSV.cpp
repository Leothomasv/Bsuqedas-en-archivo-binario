...
void OperacionesCSV::convert(){
    ...
    for(int i = 0; i < doc.GetRowCount(); i++){
              ...
              //std::memcpy(w.Date_Reported, date, sizeof(w.Date_Reported)*sizeof(date));
                ...
                w.New_cases = doc.GetCell<int>(4, index);
                w.Cumulative_cases = doc.GetCell<int>(5, index);
                w.New_deaths = doc.GetCell<int>(6, index);
                w.Cumulative_deaths = doc.GetCell<int>(7, index);
